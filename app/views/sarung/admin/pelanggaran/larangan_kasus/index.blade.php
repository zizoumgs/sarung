...
@section('content')
    <h1 class="title">
		Kasus
		<a href="{{ root::get_url_admin_larangan_kasus('add/')  }}" class="btn btn-primary btn-xs" >Add</a>
	</h1>
    @if( Session::has('message')  )
        ...
        <div class="clear_25"></div>
    @endif
	{{ Form::open(array('url' => root::get_url_admin_larangan_kasus() , 'method' => 'get' ,
        ...
        <table class="table table-striped table-hover" >
            ...
            @foreach ($items as $item)
                <tr>
                    ...
                    <td>
                        <a href="{{ root::get_url_admin_larangan_kasus('edit/'.$item->id) }}" class="btn btn-primary btn-xs" >
							<span class="glyphicon glyphicon-edit" aria-hidden="true"></span> Edit
						</a>
                        <a href="{{ root::get_url_admin_larangan_kasus('delete') }}/{{ $item->id }}" class="btn btn-danger btn-xs" >
							<span class="glyphicon glyphicon-remove-sign" aria-hidden="true"></span> Delete
						</a>
                    </td>
					<td>
						{{ $item->userobj->first_name}} {{ $item->userobj->second_name}} <br>
						{{ $item->metaObj->namaObj->nama }}
					</td>
					<td>
						Point: {{ $item->metaObj->point }} <br>
						Jenis: {{ $item->metaObj->jenis }} <br>
						Session: {{ $item->metaObj->sessionObj->nama }} <br>
					</td>
					<td>
						{{ $item->metaObj->hukuman }}
					</td>
					<td>
