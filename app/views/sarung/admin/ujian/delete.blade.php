		...
		<div class="form-group" title="Nilai yang akan di kalikan ke nilai yang di dapat siswa">
		    ...
		    <div class="col-sm-9">
				<input  name="{{ $helper::get_kelipatan_name }}" class="form-control"
						...
						require value="{{ $kelipatan }}" disabled
				>
		    </div>
		</div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
				<input type="text" name="{{ $helper::get_date_name }}" id="{{ $helper::get_date_name }}"
					   class="form-control" value="{{ $date }}" disabled
				>
            </div>
            ...
            <div class="col-sm-3">
				<input type="text" name="{{ $helper::get_time_name }}" id="{{ $helper::get_time_name }}"
					   ...
					   title="Format waktu hh:mm" value="{{$time}}" disabled
				>
            </div>
        </div>
		<hr>		
	...
	<div class="col-md-5" style="padding:15px;">
		<div class="thumbnail" style="padding:10px;">
			<h2><b>Information</b></h2><hr>
				@if ( Ujis_Model::ujianid($id)->count() > 0 )
					<span class="label label-danger">Tidak bisa menghapus</span>
					<p>Anda tidak bisa menghapus ini karena ada <span class="label label-info">{{ Ujis_Model::ujianid($id)->count() }} siswa</span>
					yang telah mengikuti ujian ini </p>
				@else
					<p>Oke anda boleh menghapus ini </p>
				@endif
