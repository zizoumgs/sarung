            ...
            <div class="col-sm-9">
				<select class="form-control" name="{{ $helper::get_kalender_name }}">
					@foreach( Kalender_Model::orderBy('created_at' , 'DESC')->get(	) as $model)
						<option value="{{ $model->id }}">{{ $model->session->nama }} | {{ $model->event->nama }} </option>
					@endforeach
				</select>
            </div>
        </div>
